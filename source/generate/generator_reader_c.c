...
static error_code_t _on_enum(generator_reader_c_t *self, const syn_enum_t *de_enum)
{
	...
	generator_printline(&self->super, 0, "{");
	generator_printline(&self->super, 0, "\ttdr_error_code_t ret = E_TDR_NOERROR;");
	generator_printline(&self->super, 0, "\tif((ret = tdr_read_enum_begin(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_enum->name);
	generator_printline(&self->super, 0, "");
	generator_print(&self->super, 0, "\tret = tdr_read_");
	...
	generator_printline(&self->super, 0, "\tif(ret == E_TDR_PLEASE_READ_ENUM_NAME)");
	generator_printline(&self->super, 0, "\t{");
	generator_printline(&self->super, 0, "\t\tchar name[TDR_MAX_LENGTH_OF_IDENTIFIER];");
	generator_printline(&self->super, 0, "\t\tif((ret = tdr_read_string(self, name, TDR_MAX_LENGTH_OF_IDENTIFIER)) != E_TDR_NOERROR) goto done;");
	...
	generator_printline(&self->super, 0, "");
	generator_printline(&self->super, 0, "\t\t\tret = E_TDR_NOT_FOUND;");
	generator_printline(&self->super, 0, "\t\t\tgoto done;");
	generator_printline(&self->super, 0, "\t\t}");
	generator_printline(&self->super, 0, "\t}");
	generator_printline(&self->super, 0, "\telse if(ret != E_TDR_NOERROR)");
	generator_printline(&self->super, 0, "\t{");
	generator_printline(&self->super, 0, "\t\tgoto done;");
	generator_printline(&self->super, 0, "\t}");
	...
	generator_printline(&self->super, 0, "");
	generator_printline(&self->super, 0, "");
	generator_printline(&self->super, 0, "\tif((ret = tdr_read_enum_end(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_enum->name);
...
static error_code_t _on_struct(generator_reader_c_t *self, const syn_struct_t *de_struct)
{
	...
	generator_printline(&self->super, 0, "{");
	generator_printline(&self->super, 0, "\ttdr_error_code_t ret = E_TDR_NOERROR;");
	generator_printline(&self->super, 0, "\tif((ret =tdr_read_struct_begin(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_struct->name);
	for(i = 0; i < de_struct->field_list.field_list_num; ++i)
	{
		...
		if(de_struct->field_list.field_list[i].type.type == E_SNT_CONTAINER)
		{
			if(de_struct->field_list.field_list[i].type.ct.ct == E_CT_VECTOR)
			{
				...
				generator_printline(&self->super, 2, "if((ret = tdr_read_vector_begin(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier);
			
				generator_printline(&self->super, 0, "");
				generator_printline(&self->super, 2, "{");
				generator_printline(&self->super, 3, "if((ret = tdr_read_field_begin(self, \"%s_num\")) == E_TDR_NOERROR)", de_struct->field_list.field_list[i].identifier);
				generator_printline(&self->super, 3, "{");
				generator_print(&self->super, 4, "if((ret = tdr_read_");
				generator_print_type_name(&self->super, &g_vec_num_type);
				generator_printline(&self->super, 0, "(self, &data->"VEC_NUM_TYPE_STYLE")) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier);
				generator_printline(&self->super, 3, "}");
				generator_printline(&self->super, 3, "else if(ret != E_TDR_IGNORE)");
				generator_printline(&self->super, 4, "goto done;");
				generator_printline(&self->super, 3, "if((ret = tdr_read_field_end(self, \""VEC_NUM_TYPE_STYLE"\")) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier);
				generator_printline(&self->super, 2, "}");
				generator_printline(&self->super, 0, "");
				...
				generator_printline(&self->super, 0, "");
				generator_printline(&self->super, 3, "if((ret = tdr_read_vector_element_begin(self, \"%s\", i)) == E_TDR_NOERROR)", de_struct->field_list.field_list[i].identifier);				
				generator_printline(&self->super, 3, "{");
				if(vector_type->st == E_ST_STRING)
				{
					generator_printline(&self->super, 4, "if((ret = tdr_read_string(self, data->%s[i], %s)) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier, vector_type->string_length);
				}
				else
				{
					generator_print(&self->super, 4, "if((ret = tdr_read_");
					...
					generator_printline(&self->super, 0, ")) != E_TDR_NOERROR) goto done;");
				}				
				generator_printline(&self->super, 3, "}");
				generator_printline(&self->super, 3, "else if(ret != E_TDR_IGNORE)");
				generator_printline(&self->super, 4, "goto done;");
				generator_printline(&self->super, 3, "if((ret = tdr_read_vector_element_end(self, \"%s\", i)) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier);
				generator_printline(&self->super, 0, "");
				generator_printline(&self->super, 2, "}");				

				generator_printline(&self->super, 2, "if((ret = tdr_read_vector_end(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier);
			}
		}
		else if(de_struct->field_list.field_list[i].type.type == E_SNT_SIMPLE)
		{
			...
			generator_printline(&self->super, 2, "if((ret = tdr_read_field_begin(self, \"%s\")) == E_TDR_NOERROR)", de_struct->field_list.field_list[i].identifier);			
			generator_printline(&self->super, 2, "{");
			if(st->st == E_ST_STRING)
			{
				generator_printline(&self->super, 3, "if((ret = tdr_read_string(self, data->%s, %s)) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier, de_struct->field_list.field_list[i].type.st.string_length);
			}
			else
			{
				generator_print(&self->super, 3, "if((ret = tdr_read_");
			...
			generator_printline(&self->super, 2, "}");
			generator_printline(&self->super, 2, "else if(ret != E_TDR_IGNORE)");
			generator_printline(&self->super, 3, "goto done;");
			generator_printline(&self->super, 2, "if((ret = tdr_read_field_end(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_struct->field_list.field_list[i].identifier);
	...
	generator_printline(&self->super, 0, "");
	generator_printline(&self->super, 0, "\tif((ret = tdr_read_struct_end(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_struct->name);
...
static error_code_t _on_union(generator_reader_c_t *self, const syn_union_t *de_union)
{
	...
	generator_printline(&self->super, 0, "tdr_error_code_t tdr_read_%s(tdr_abstract_reader_t *self, union %s *data, enum %s selector)", de_union->name, de_union->name, de_union->parameters.par_list[0].type.st_refer);
	generator_printline(&self->super, 0, "{");
	generator_printline(&self->super, 0, "\ttdr_error_code_t ret = E_TDR_NOERROR;");
	generator_printline(&self->super, 0, "\tif((ret = tdr_read_union_begin(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_union->name);
	...
	for(i = 0; i < de_union->union_field_list.union_field_list_num; ++i)
	{
		...
		generator_printline(&self->super, 2, "if((ret = tdr_read_field_begin(self, \"%s\")) == E_TDR_NOERROR)", de_union->union_field_list.union_field_list[i].name);
		generator_printline(&self->super, 2, "{");
		if(st->st == E_ST_STRING)
		{
			generator_printline(&self->super, 3, "if((ret = tdr_read_string(self, data->%s, %s) != E_TDR_NOERROR) goto done;", de_union->union_field_list.union_field_list[i].name, de_union->union_field_list.union_field_list[i].simple_type.string_length);
		}
		else
		{
			generator_print(&self->super, 3, "if((ret = tdr_read_");
			generator_print_type_name(&self->super, st);
			generator_printline(&self->super, 0, "(self, &data->%s)) != E_TDR_NOERROR) goto done;", de_union->union_field_list.union_field_list[i].name);
		}
		generator_printline(&self->super, 2, "}");
		generator_printline(&self->super, 2, "else if(ret != E_TDR_IGNORE)");
		generator_printline(&self->super, 3, "goto done;");
		generator_printline(&self->super, 2, "if((ret = tdr_read_field_end(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_union->union_field_list.union_field_list[i].name);
	...
	generator_printline(&self->super, 0, "\t}");
	generator_printline(&self->super, 0, "\tif((ret = tdr_read_union_end(self, \"%s\")) != E_TDR_NOERROR) goto done;", de_union->name);
...
{
	generator_reader_c_t *self = TDR_CONTAINER_OF(super, generator_reader_c_t, super);
	TDR_UNUSED(yylloc);
