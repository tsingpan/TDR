...
static TD_ERROR_CODE _on_enum(TLIBC_READER_GENERATOR *self, const ST_ENUM *de_enum)
{
	...
	generator_print(&self->super, "\n");
	generator_print(&self->super, "\n");
	generator_print(&self->super, "\tif((ret = tlibc_read_enum_end(self, \"%s\")) != E_TLIBC_NOERROR) goto done;\n", de_enum->name);
...
static TD_ERROR_CODE _on_struct(TLIBC_READER_GENERATOR *self, const ST_STRUCT *de_struct)
{
	...
	generator_print(&self->super, "{\n");
	generator_print(&self->super, "\tTLIBC_ERROR_CODE ret = E_TLIBC_NOERROR;\n");
	generator_print(&self->super, "\tif((ret =tlibc_read_struct_begin(self, \"%s\")) != E_TLIBC_NOERROR) goto done;\n", de_struct->name);
	for(i = 0; i < de_struct->field_list.field_list_num; ++i)
	{
		...
		if(de_struct->field_list.field_list[i].type.type == E_SNT_CONTAINER)
		{
			if(de_struct->field_list.field_list[i].type.ct.ct == E_CT_VECTOR)
			{
				...
				{
					generator_print(&self->super, "\t\t\tif((ret = tlibc_read_");
				...
				generator_print(&self->super, "\t\t\tif((ret = tlibc_read_vector_element_end(self, \"%s\", i)) != E_TLIBC_NOERROR) goto done;\n", de_struct->field_list.field_list[i].identifier);
				generator_print(&self->super, "\t\t}\n");				

				generator_print(&self->super, "\t\tif((ret = tlibc_read_vector_end(self)) != E_TLIBC_NOERROR) goto done;\n");
			}
		}
		else if(de_struct->field_list.field_list[i].type.type == E_SNT_SIMPLE)
		{
			...
			generator_print(&self->super, "\t\tif((ret = tlibc_read_field_end(self, \"%s\")) != E_TLIBC_NOERROR) goto done;\n", de_struct->field_list.field_list[i].identifier);
		}

		generator_print(&self->super, "\t}\n");
	}
	generator_print(&self->super, "\n");
	generator_print(&self->super, "\tif((ret = tlibc_read_struct_end(self, \"%s\")) != E_TLIBC_NOERROR) goto done;\n", de_struct->name);
...
static TD_ERROR_CODE _on_union(TLIBC_READER_GENERATOR *self, const ST_UNION *de_union)
{
	...
	generator_print(&self->super, "{\n");
	generator_print(&self->super, "\tTLIBC_ERROR_CODE ret = E_TLIBC_NOERROR;\n");
	generator_print(&self->super, "\tif((ret = tlibc_read_union_begin(self, \"%s\")) != E_TLIBC_NOERROR) goto done;\n", de_union->name);
	...
	generator_print(&self->super, "\t}\n");
	generator_print(&self->super, "\tif((ret = tlibc_read_union_end(self, \"%s\")) != E_TLIBC_NOERROR) goto done;\n", de_union->name);
